...
{
    Data::sendPrivateMsg($user_id, $raw_message);
}
...
function GroupAdminEvent($time, $self_id, $post_type, $notice_type, $sub_type, $group_id, $user_id)
{
    //do some thing
}

function GroupBanEvent($time, $self_id, $post_type, $notice_type, $sub_type, $group_id, $operator_id, $user_id, $duration)
{
    //do some thing
}

function FriendAddEvent($time, $self_id, $post_type, $notice_type, $user_id)
{
    //do some thing
}

function FriendRequestEvent($quick, $time, $self_id, $post_type, $request_type, $user_id, $comment, $flag)
{
    //Data::setFriendAdd($flag);
}

function GroupMemberAddRequestEvent($quick, $time, $self_id, $post_type, $request_type, $sub_type, $group_id, $user_id, $comment, $flag)
{
    //do some thing
}

//API回调参数请访问go-cqhttp标准: https://docs.go-cqhttp.org/api
    ...
    if($json['status'] == 'ok')
    {
        ...
        /*if (empty($data['ClassType'])) {
            foreach ($data as $group) {
                if ($group['ClassType'] == "GroupData") {//get_group_list()回调
                    global $qq;
                    if(count($qq->query("select * from `information_schema`.tables where `table_schema` = 'qq' and `table_name` = '{$group['group_id']}'")) == 0){
                        $qq->query("CREATE TABLE `qq`.`{$group['group_id']}` (
                                `message_id` BIGINT NOT NULL AUTO_INCREMENT , `user_id` BIGINT NOT NULL ,
                                `user_name` VARCHAR(150) NOT NULL , `message` TEXT NOT NULL ,
                                `time` BIGINT NOT NULL , PRIMARY KEY (`message_id`),
                                INDEX `user_id` (`user_id`), INDEX `time` (`time`), FULLTEXT `message` (`message`)) ENGINE = InnoDB;"
                        );
                    }
                }
            }
